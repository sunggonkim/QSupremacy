...
\begin{figure}[t]
    ...
    \Description{Block diagram of QSupremacy with shared workload control, native path execution, quantum path execution, measurement logging, and advantage threshold analysis.}
...
Different application families expose different quality values, so \SystemName stores both the native metric and a tolerance-scaled gap. For higher-is-better metrics, the gap is the native score minus the quantum score. For lower-is-better metrics, it is the quantum error minus the native/reference error. \SystemName compares the residual gap to the workload tolerance $\epsilon_w$:
\[
