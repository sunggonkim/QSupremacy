...
The native path trains and evaluates classical models directly on the preprocessed features. For ML, \SystemName now supports softmax regression, MLP, linear ridge classification, RBF kernel ridge classification, k-nearest neighbors, and nearest-centroid baselines without requiring an external ML package. For optimization, \SystemName records exact enumeration when feasible and also greedy, local-search, and simulated-annealing baselines. The native time used in the threshold is the fastest implemented model that reaches the best native quality within a small tolerance. This rule prevents a weak or low-quality native model from making the quantum path look artificially competitive.
...
\subsection{Workload Expansion Plan}

Table~\ref{tab:workload-suite} shows the intended workload suite. The current paper has measured the first row. The remaining rows are required before making broad claims about practical quantum supremacy.

\begin{table}[t]
\caption{Planned practical workload suite.}
\centering
\small
