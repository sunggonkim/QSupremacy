...
\SystemName is designed to answer one question: when does a quantum-circuit application beat the native HPC/ML application for the same task? Figure~\ref{fig:design-overview} summarizes the key components. First, \SystemName uses \textit{Shared Workload Control} to ensure that all paths use the same data split and preprocessing. Second, it uses \textit{Native Path Execution} to establish a strong classical baseline. Third, it uses \textit{Quantum Path Execution} to run quantum kernel and QNN/VQC models with \cuQuantum. Finally, it uses \textit{Hardware Projection} to estimate the future quantum hardware requirements.
...
Figure~\ref{fig:design-overview} shows the overall procedure. \SystemName has two main phases: \textit{Measurement} and \textit{Projection}.
