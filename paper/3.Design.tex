...
The evaluation also sweeps $P_{\mathrm{shots}}^{\mathrm{eff}}$ under this stack and leaves decoder/control terms serial, so the analysis cannot hide all overhead behind shot parallelism.
...
\begin{tabular}{llll}
\toprule
\textbf{Family} & \textbf{Native path} & \textbf{Circuit path} & \textbf{Quality target} \\
\midrule
...
\label{tab:design-workload-paths}
\end{table}
