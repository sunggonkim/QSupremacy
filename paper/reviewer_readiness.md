...
| Worked hardware scenario | The hardware frontier is too notional without at least one concrete stack. | The design now gives an illustrative surface-code-style instantiation with code distance, cycle time, logical-operation multipliers, decoder latency, and shot parallelism. | `paper/3.Design.tex`, `paper/references.bib` |
...
| Tolerance sensitivity | The advantage frontier may depend materially on tolerance choices. | The design and evaluation now state the per-family tolerances, treat them as model inputs, and align taxonomy tolerances with projection tolerances. | `paper/3.Design.tex`, `paper/4.Evaluation.tex`, `scripts/generate_workload_taxonomy.py` |
...
| QHPC workflow context | The related work may miss scheduling/orchestration perspectives. | The related work now distinguishes the paper from hybrid quantum-classical schedulers and orchestration frameworks. | `paper/5.RelatedWork.tex`, `paper/references.bib` |
...
| ML qubit provenance | The digits plots may appear to imply a different qubit scale than the PCA dimensions. | The evaluation now states that digits uses one qubit per PCA feature and that the plots come from the 4/8/12/16-qubit measured records. | `paper/4.Evaluation.tex` |
| Name sensitivity | The old project name may be confused with quantum-supremacy claims. | The manuscript display name is now the neutral `QAdvantage`; the repository name remains historical. | `paper/0.Main.tex`, `paper/5.Discussion.tex` |
...
| Line-by-line response | Reviewer concerns may be addressed only diffusely. | A separate response map links each pasted concern to manuscript edits, artifacts, or explicit follow-up scope. | `paper/reviewer_line_by_line_response.md` |
